...
If you R version is < 4.1, please [download and install the latest version of R](https://cran.r-project.org/mirrors.html), and then restart your R.
...
And the `update_tidymass()` function can be used to update `tidymass` and packages in it.

```{r,eval=TRUE,warning=FALSE, R.options="", message=FALSE, cache=TRUE}
update_tidymass(from = "github")
```
