...
Data introduction. We have RPLC (positive and negative mode), HILIC (positive and negative mode).
...
```{r,eval=TRUE,warning=FALSE, R.options="", message=TRUE, cache=TRUE}
sample_info_pos <- readr::read_csv("sample_info/RPLC/sample_info_pos_1.csv")
...
Save thme for subsequent analysis.
...
```{r,eval=TRUE,warning=FALSE, R.options="", message=TRUE, cache=TRUE}
head(colnames(object_rplc_pos2))
```

```{r,eval=TRUE,warning=FALSE, R.options="", message=TRUE, cache=TRUE}
head(colnames(object_rplc_neg2))
```

```{r,eval=TRUE,warning=FALSE, R.options="", message=TRUE, cache=TRUE}
head(colnames(object_hilic_pos2))
```

```{r,eval=TRUE,warning=FALSE, R.options="", message=TRUE, cache=TRUE}
head(colnames(object_hilic_neg2))
```
...
volcano_plot(object = object,
  ...
  scale_size_continuous(range = c(0.5, 5))
plot
ggsave(plot, filename = "statistical_analysis/male_LCC_vs_femal_control.pdf", width = 8, height = 7)
```
